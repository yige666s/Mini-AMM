...
func main() {
	...
	compoundService, err := services.NewCompoundService(config, rpcClient, txService)
	if err != nil {
		log.Fatalf("初始化复投服务失败: %v", err)
	}

	rebalanceService, err := services.NewRebalanceService(config, rpcClient, txService, compoundService)
	if err != nil {
