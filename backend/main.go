...
func main() {
	...
	rebalanceService, err := services.NewRebalanceServiceMarket(config, rpcClient, txService, compoundService, botActionRepo)
	if err != nil {
	...
	apiServer := api.NewServer(apiPort, botActionRepo, config)
	go func() {
